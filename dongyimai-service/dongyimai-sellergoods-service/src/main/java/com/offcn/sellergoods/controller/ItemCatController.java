...
public class ItemCatController {
    ...
    @ApiOperation(value = "根据prentID查询列表", notes = "根据prentID查询列表详情", tags = {"ItemCatController"})
    @ApiImplicitParam(paramType = "path", name = "id", value = "父类ID", required = true, dataType = "Long")
    @GetMapping(value = "findByParentId/{id}")
    public Result<List<ItemCat>> findByParentId(@PathVariable Long id) {
        List<ItemCat> itemCats = itemCatService.findByParentId(id);
        return new Result(true, StatusCode.OK, "查询成功", itemCats);
    }
}
